...
class ResPartner(models.Model):
    ...
    def check_vat(self):
        ...
        l10n_ve_partners = self.filtered(lambda x: x.country_id.code == 'VE')
        person_vat_pattern = "^[0-9]{1,9}$"
        enterprise_vat_pattern = "^[0-9]{9}$"
        for partner in l10n_ve_partners:
            if not partner.prefix_vat:
                raise UserError(_("Debe indicar el tipo de CI/RIF"))
            
            if partner.prefix_vat in ('V', 'E'):
                if not (re.match(person_vat_pattern, partner.vat)):
                    raise UserError(_("The vat field only accepts numbers and must be between 1 and 9 digits"))
            elif not re.match(enterprise_vat_pattern, partner.vat):
                raise UserError(_("The vat field only accepts numbers and must be 9 digits long"))
